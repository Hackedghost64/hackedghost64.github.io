...
export class PlayerManager {
    ...
    private icons = {
        ...
        settings: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-index="20" height="20"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1 0 2.83 2 2 0 0 1-2.83 0l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-2 2 2 2 0 0 1-2-2v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83 0 2 2 0 0 1 0-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1-2-2 2 2 0 0 1 2-2h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 0-2.83 2 2 0 0 1 2.83 0l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 2-2 2 2 0 0 1 2 2v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 0 2 2 0 0 1 0 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 2 2 2 2 0 0 1-2 2h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg>',
    ...
    play(options: PlaybackOptions) {
        ...
        // Be very permissive about direct files to ensure custom UI loads
        ...
        if (isDirectFile && !options.isEmbed) {
            console.log("[Player] Mounting native player UI");
            this.mountNativePlayer(options.url, options.isHls);
            this.injectModernControls(options);
            this.setupInteractions();
        } else {
            console.log("[Player] Mounting iframe (embed mode)");
            this.mountIframe(options.url);
        }
    }

    private mountNativePlayer(url: string, isHls: boolean) {
    ...
    private injectModernControls(options: PlaybackOptions) {
        ...
        leftGroup.className = 'player-control-group';
        ...
        timeDisplay.className = 'text-[10px] md:text-xs font-bold font-mono tracking-tighter opacity-80';
        timeDisplay.id = 'player-time';
        timeDisplay.textContent = '00:00 / 00:00';
        leftGroup.appendChild(timeDisplay);

        const rightGroup = document.createElement('div');
        rightGroup.className = 'player-control-group';
        ...
        // Secondary buttons - visible on MD+, hidden on mobile (moved to settings)
    ...
    private setupControlListeners(seeker: HTMLInputElement, volSlider: HTMLInputElement) {
    ...
    private cycleSpeed(btn: HTMLButtonElement) {
        if (!this.videoElement) return;
    ...
    private showSkipIndicator(text: string, side: string) {
    ...
    private triggerCast() {
        if (this.videoElement && (this.videoElement as any).remote) (this.videoElement as any).remote.prompt();
    ...
    private togglePiP() {
        if (this.videoElement) {
             ...
             else this.videoElement.requestPictureInPicture();
        }
    }
    ...
    private toggleSettings() {
        ...
        this.currentOptions.allLinks.forEach((link, idx) => {
        ...
        // Mobile Quick Actions (only show if screen is small)
        if (window.innerWidth < 768) {
            const actionsTitle = document.createElement('div');
            actionsTitle.className = 'text-[10px] font-black uppercase tracking-widest p-2 opacity-40 border-t border-white/5 mt-1';
            actionsTitle.textContent = 'Quick Actions';
            menu.appendChild(actionsTitle);

            const vlcMobile = document.createElement('button');
            vlcMobile.className = 'flex items-center gap-3 p-3 rounded-xl text-xs font-bold hover:bg-white/5';
            vlcMobile.innerHTML = `${this.icons.vlc} Open in VLC`;
            vlcMobile.onclick = () => { window.location.href = `vlc://${this.currentOptions?.url}`; menu.remove(); };
            menu.appendChild(vlcMobile);

            const skipMobile = document.createElement('button');
            skipMobile.className = 'flex items-center gap-3 p-3 rounded-xl text-xs font-bold hover:bg-white/5';
            skipMobile.innerHTML = `${this.icons.skipForward} Skip Intro (+85s)`;
            skipMobile.onclick = () => { if (this.videoElement) this.videoElement.currentTime += 85; menu.remove(); };
            menu.appendChild(skipMobile);
        }

        this.container.appendChild(menu);
    }

    private switchQuality(link: StreamingLink) {
        ...
        // Re-mount without clearing full UI
    ...
    private createIconButton(icon: string, onClick: () => void, title: string): HTMLElement {
    ...
    private mountIframe(url: string) {
        this.clear();
